...
import { MapPin, Hospital, Shield, Building2, Heart, Star, Users, Phone, Mail, MapPinIcon, Plus, Minus } from 'lucide-react';
...
// Professional custom marker icons for medical facilities
const createCustomIcon = (type: 'clinical' | 'non-clinical' | 'military' | 'clinic', isHighlighted = false) => {
  const configs = {
    clinical: { 
      color: '#1E40AF', 
      bgColor: '#DBEAFE',
...
const clinicIcon = createCustomIcon('clinic');
...
const clinicIconHighlight = createCustomIcon('clinic', true);

type SortOption = 'default' | 'alphabetical' | 'rating-desc' | 'rating-asc' | 'positions-desc' | 'positions-asc';
...
const AddFacilityModal: React.FC<AddFacilityModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('New facility data:', formData);
    alert('Dziękujemy za zgłoszenie placówki! Zostanie zweryfikowana przez nasz zespół.');
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-bold text-gray-900">Dodaj nową placówkę</h3>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            >
              ...
              {medicalSpecializations.slice(1).map(spec => (
          ...
          <div>
            ...
            >
              ...
              <option value="clinical">Szpital kliniczny</option>
              <option value="non-clinical">Szpital niekliniczny</option>
              <option value="military">Szpital wojskowy</option>
            </select>
          </div>
...
interface ResidencyMapProps {
  darkMode?: boolean;
  fontSize?: 'small' | 'medium' | 'large';
}

const ResidencyMap: React.FC<ResidencyMapProps> = ({ darkMode, fontSize }) => {
  ...
  useEffect(() => {
    if (!loading && !isDoctor) {
      console.warn('Access denied: Only doctors can access the residency map');
    }
  }, [profile, loading, isDoctor]);

  // Filter facilities based on selected filters
  const filteredFacilities = useMemo(() => {
    let filtered = trainingFacilities;

    // Filter by specialization
    if (selectedSpecialization && selectedSpecialization !== 'Wszystkie specjalizacje') {
      filtered = filtered.filter(facility => 
        facility.specializations.some(spec => spec.name === selectedSpecialization)
      );
    }
    ...
    return filtered;
  }, [selectedSpecialization, selectedVoivodeships, searchQuery]);
    ...
    switch (sortBy) {
      ...
      case 'positions-desc':
        return sorted.sort((a, b) => {
          const aTotalPositions = a.specializations.reduce((sum, spec) => sum + spec.totalPositions, 0);
          const bTotalPositions = b.specializations.reduce((sum, spec) => sum + spec.totalPositions, 0);
          return bTotalPositions - aTotalPositions;
        });
      case 'positions-asc':
        return sorted.sort((a, b) => {
          const aTotalPositions = a.specializations.reduce((sum, spec) => sum + spec.totalPositions, 0);
          const bTotalPositions = b.specializations.reduce((sum, spec) => sum + spec.totalPositions, 0);
          return aTotalPositions - bTotalPositions;
        });
      default:
        return sorted;
    }
  ...
  const getMarkerIcon = (type: 'clinical' | 'non-clinical' | 'military' | 'clinic', facilityId: number, isHighlighted = false) => {
    ...
    switch (type) {
      ...
      case 'clinic': return shouldHighlight ? clinicIconHighlight : clinicIcon;
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md text-center">
          ...
          <p className="text-gray-600">
            Mapa rezydentur dostępna tylko dla lekarzy.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Mapa rezydentur</h1>
            <p className="mt-2 text-gray-600 max-w-4xl">
              W tym miejscu znajdziesz mapę wszystkich jednostek szkoleniowych wraz z opiniami na ich temat. 
              Sprawdź wykaz wolnych miejsc na tryb rezydencki i pozarezydencki oraz całkowita liczbę miejsc 
              szkoleniowych zatwierdzona przez CMKP.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 rounded-xl shadow-md border border-green-100">
              ...
              <input
                ...
                placeholder="Wyszukaj placówkę lub oddział..."
                className="w-full p-2.5 bg-white border rounded-lg focus:ring-2 focus:border-transparent transition-all duration-200"
                style={{borderColor: '#38b6ff'}}
                onFocus={(e) => e.currentTarget.style.boxShadow = '0 0 0 2px rgba(56, 182, 255, 0.3)'}
                onBlur={(e) => e.currentTarget.style.boxShadow = ''}
            ...
            {/* Add Facility Panel */}
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 rounded-xl shadow-md border border-amber-100">
              <p className="text-sm text-gray-700 mb-3">Brakuje Twojej placówki?</p>
              <button
                onClick={() => setIsAddModalOpen(true)}
                className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white px-4 py-2.5 rounded-lg hover:from-amber-600 hover:to-orange-600 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
              >
                Dodaj
              </button>
            </div>
            ...
            <div className="bg-gradient-to-br from-gray-50 to-slate-50 p-4 rounded-xl shadow-md border border-gray-200">
              ...
              >
                ...
                <option value="positions-desc">Miejsca: rosnąco</option>
                <option value="positions-asc">Miejsca: malejąco</option>
              </select>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-100">
              <div className="p-4 border-b border-gray-200">
                <h3 className="text-lg font-medium text-gray-900">
                  Placówki szkoleniowe ({sortedFacilities.length})
                </h3>
              </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-600 mb-3">
                          <MapPin className="w-3 h-3" style={{color: '#38b6ff'}} />
                        ...
                        <div className="space-y-2">
                          {facility.specializations.slice(0, 2).map((spec, index) => (
                            <div key={index} className="p-2 bg-white rounded-lg border border-gray-200 shadow-sm">
                              <div className="text-xs font-medium mb-1" style={{color: '#38b6ff'}}>
                                {spec.name}
                              </div>
                              ...
                              <div className="flex items-center justify-between text-xs">
                                <span className="text-gray-500">Miejsca:</span>
                                <span className="font-semibold text-green-600">
                                  <Users className="w-3 h-3 inline mr-1" />
                                  {spec.currentPositions === 0 && spec.totalPositions === 0 ? 'b.d.' : `${spec.currentPositions}/${spec.totalPositions}`}
                                </span>
                              </div>
                            </div>
                          ))}
                          {facility.specializations.length > 2 && (
                            <div className="text-xs text-center p-1" style={{color: '#38b6ff'}}>
                              +{facility.specializations.length - 2} więcej specjalizacji
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  {sortedFacilities.map((facility) => (
                    ...
                    >
                      <Popup maxWidth={400} className="custom-popup">
                        <div className="bg-gradient-to-br from-white to-gray-50 p-4 rounded-lg">
                          {/* Header */}
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <h3 className="font-bold text-lg text-gray-900 mb-1 leading-tight">{facility.name}</h3>
                              <div className="flex items-center gap-2 text-sm text-gray-600">
                                <MapPin className="w-4 h-4" style={{color: '#38b6ff'}} />
                                <span>{facility.city}</span>
                              </div>
                            </div>
                            <button
                            ...
                            <div className="max-h-40 overflow-y-auto space-y-2">
                              {facility.specializations.map((spec, index) => (
                                <div key={index} className="p-3 bg-white rounded-lg border border-gray-200 shadow-sm">
                                  <div className="font-medium mb-1" style={{color: '#38b6ff'}}>{spec.name}</div>
                                  <div className="text-sm text-gray-600 mb-2">{spec.department}</div>
                                  <div className="flex items-center justify-between">
                                    <span className="text-xs text-gray-500">Miejsca szkoleniowe:</span>
                                    <span className="text-sm font-semibold text-green-600">
                                      {spec.currentPositions === 0 && spec.totalPositions === 0 
                                        ? 'Brak danych' 
                                        : `${spec.currentPositions} / ${spec.totalPositions}`
                                      }
                                    </span>
                                  </div>
                                </div>
      ...
      {/* Custom CSS for map enhancements */}
      <style jsx global>{`
        .medical-marker-icon {
