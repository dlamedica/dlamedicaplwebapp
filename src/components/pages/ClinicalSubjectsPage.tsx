...
import { 
  ...
  nurseClinicalSubjects, 
  physiotherapistClinicalSubjects, 
  pharmacistClinicalSubjects,
  dietitianClinicalSubjects,
  psychologistClinicalSubjects,
  paramedicClinicalSubjects,
  remainingClinicalSubjects,
  allClinicalSubjects,
  getClinicalSubjectsForProfession
} from '../../data/clinicalSubjects';
import { FaChevronDown, FaChevronUp } from 'react-icons/fa';

interface ClinicalSubjectsPageProps {
  darkMode: boolean;
  fontSize: 'small' | 'medium' | 'large';
}

const ClinicalSubjectsPage: React.FC<ClinicalSubjectsPageProps> = ({ darkMode, fontSize }) => {
  ...
  useEffect(() => {
    document.title = 'Przedmioty Kliniczne – DlaMedica.pl';
    
    // Load favorites from localStorage
    const savedFavorites = localStorage.getItem('favoriteClinicalSubjects');
    if (savedFavorites) {
  ...
  const handleFavoriteToggle = (subjectId: number) => {
    ...
    localStorage.setItem('favoriteClinicalSubjects', JSON.stringify(newFavorites));
  };
  ...
  const professionalSubjects = getClinicalSubjectsForProfession(userProfession);
  
  // Get favorite subjects
  const favoriteSubjects = allClinicalSubjects.filter(s => favorites.includes(s.id));
  
  // Sekcja "Pozostałe przedmioty" została wyłączona

  return (
    <MainLayout darkMode={darkMode} showSidebar={true} currentPage="clinical-subjects">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className={`${fontSizes.subtitle} ${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
              Specjalizacje medyczne, chirurgia, diagnostyka i leczenie kliniczne
            </p>
...
export default ClinicalSubjectsPage;